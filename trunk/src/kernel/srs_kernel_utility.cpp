...
bool srs_string_icase_compare(std::string s1, std::string s2)
{
    return s1.size() == s2.size() ? std::equal(s1.begin(), s1.end(), s2.begin(), [](int a, int b) { return tolower(a) == tolower(b); }) : false;
}
...
namespace
{
    const EVP_MD* (*digest_routines[])() = { &EVP_sha1, &EVP_sha224, &EVP_sha256, &EVP_sha384, &EVP_sha512,
    ...
    const EVP_MD* make_digest_routine(SrsDigestMode mode)
    {
        return digest_routines[static_cast<std::size_t>(mode)]();
    }
}

srs_error_t make_digest(SrsDigestMode mode, std::string buffer, std::string& result)
{
    ...
    if (!EVP_DigestInit_ex(context, make_digest_routine(mode), nullptr)) {
        return srs_error_new(-1, "Failed to initialize the digest.");
    }

    if (!EVP_DigestUpdate(context, buffer.data(), buffer.size())) {
        return srs_error_new(-2, "Failed to update the digest.");
    }
    ...
    if (!EVP_DigestFinal_ex(context, hash, &hash_size)) {
        return srs_error_new(-3, "Failed to finalize the digest.");
    }

    result.resize(hash_size * 2);
    srs_data_to_hex_lowercase(&result[0], hash, static_cast<int>(hash_size));
